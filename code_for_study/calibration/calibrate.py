...
TERMINATION_CRITERIA = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_MAX_ITER, 22,
        0.001)

imageDrawCornerR = "/Users/admin/Desktop/Robot/EIIC/outputR/{:06d}.png"
imageDrawCornerL = "/Users/admin/Desktop/Robot/EIIC/outputL/{:06d}.png"
# MAX_IMAGES = 64
"""
if len(sys.argv) != 4:
...
"""
leftImageDir = "/Users/admin/Desktop/Robot/EIIC/left"
rightImageDir = "/Users/admin/Desktop/Robot/EIIC/right"
outputFile = "/Users/admin/Desktop/Robot/EIIC/output"
def readImagesAndFindChessboards(imageDirectory, imageDraw):
    ...
    for imagePath in sorted(imagePaths):
        ...
        cv2.imwrite(imageDraw.format(frameOut), image)
        frameOut += 1
...
(leftFilenames, leftObjectPoints, leftImagePoints, leftSize
        ) = readImagesAndFindChessboards(leftImageDir, imageDrawCornerL)
(rightFilenames, rightObjectPoints, rightImagePoints, rightSize
        ) = readImagesAndFindChessboards(rightImageDir, imageDrawCornerR)
...
for i in  leftImage:
    img = cv2.imread(i)
    h,  w = img.shape[:2]
    newcameramtx, roi=cv2.getOptimalNewCameraMatrix(leftCameraMatrix,leftDistortionCoefficients,(w,h),1,(w,h))
    # undistort
    leftDistortionCoefficients = cv2.undistort(img, leftCameraMatrix, leftDistortionCoefficients, None, newcameramtx)
    # crop the image
    x,y,w,h = roi
...
rightImage = glob.glob("/Users/admin/Desktop/Robot/EIIC/right/*.png")
rightImage = sorted(rightImage)
saveRight = "/Users/admin/Desktop/Robot/EIIC/outputCalibratedR/{:06d}.png"
frameRight = 1
for i in  rightImage:
    img = cv2.imread(i)
    h,  w = img.shape[:2]
    newcameramtx, roi=cv2.getOptimalNewCameraMatrix(rightCameraMatrix,rightDistortionCoefficients,(w,h),1,(w,h))
    # undistort
    dst = cv2.undistort(img, rightCameraMatrix, rightDistortionCoefficients, None, newcameramtx)
    # crop the image
    x,y,w,h = roi
    rightDistortionCoefficients = rightDistortionCoefficients[y:y+h, x:x+w]
    cv2.imwrite(saveRight.format(frameRight),rightDistortionCoefficients)
    frameRight += 1
